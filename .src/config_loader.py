...
def validate_config(config: dict) -> None:
    ...
    if "default_date" in config and not isinstance(config["default_date"], str):
        raise ValueError("default_date 必须是字符串。")
    if "enable_default_date" in config and not isinstance(config["enable_default_date"], bool):
        raise ValueError("enable_default_date 必须是 true 或 false。")
