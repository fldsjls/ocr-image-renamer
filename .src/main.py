...
DEFAULT_CONFIG = {
    ...
    # 文件名模板。{area} 和 {content} 会被替换成识别出来的值。
    "filename_template": "{area}_{content}",
    # 文件夹模板。斜杠 / 表示创建下一层文件夹。
    "folder_template": "{area}/{content}",
...
def load_config(path: Path) -> dict:
    ...
    for field in config["fields"]:
        if "key" not in field or "label" not in field:
            raise ValueError("fields 里的每一项都必须有 key 和 label。")
...
def extract_values(text: str, config: dict) -> dict[str, str]:
    ...
    labels = [field["label"] for field in config["fields"]]
    ...
    for field in config["fields"]:
        key = field["key"]
        label = field["label"]
        fallback = field.get("fallback", f"未识别{label}")
